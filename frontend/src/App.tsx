...
import type { TodoItem } from './types/todo-api';
import './App.css';

type AuthStatus = 'checking' | 'authenticated' | 'unauthenticated';

function toRouterPath(pathValue: string): string {
  if (pathValue === '/') {
    return '';
  }
...
function TodoWorkspace({ apiClient }: { apiClient: TodoApiClient }) {
...
function HomePage({
  ...
  if (!apiClient) {
    return <StatusPanel title="初期化エラー" message="API クライアントの初期化に失敗しました。" />;
  }
...
function App() {
  ...
  const todoApiClient = useMemo<TodoApiClient | null>(() => {
    if (!runtimeConfig) {
      return null;
    }
