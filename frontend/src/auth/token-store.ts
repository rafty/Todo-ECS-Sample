...
const authTokenState: AuthTokenState = {
  accessToken: '',
...
function readRefreshTokenFromSessionStorage(): string {
  try {
...
function writeRefreshTokenToSessionStorage(refreshToken: string): void {
  try {
    if (refreshToken) {
...
function resolveAccessTokenExpiry(accessToken: string): number {
...
export function configureTokenStore({ persistRefreshToken }: { persistRefreshToken: boolean }): void {
...
export function updateTokenSet(tokenResponse: OAuthTokenResponse): void {
...
export function hasValidAccessToken(expirySkewMs = 30 * 1000): boolean {
