...
function base64UrlEncode(sourceBytes) {
  const binaryString = String.fromCharCode(...sourceBytes);
  const base64String = btoa(binaryString);
  return base64String.replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/g, '');
}

function createRandomUrlSafeValue(byteLength) {
...
function persistPkceTransaction(state, codeVerifier) {
...
export function consumePkceCodeVerifier(expectedState) {
